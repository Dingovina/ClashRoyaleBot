...
from src.perception.screen_layout import ScreenLayoutReference, load_screen_layout_reference
...
class CardModelRunner:
    ...
    def infer_hand_cards(
        ...
        arr = torch.frombuffer(bytearray(pixels_bgra), dtype=torch.uint8).reshape(frame_height, frame_width, 4)[:, :, :3]
        out: list[tuple[str, float]] = []
        for rect in self._layout.hand_cards:
            l = max(0, min(rect.left, frame_width - 1))
            t = max(0, min(rect.top, frame_height - 1))
            r = min(frame_width - 1, rect.right)
            b = min(frame_height - 1, rect.bottom)
            if r < l or b < t:
                out.append(("unknown", 0.0))
                continue
            crop_rgb = arr[t : b + 1, l : r + 1, [2, 1, 0]].cpu().numpy()
            ...
            x = torch.frombuffer(bytearray(image.tobytes()), dtype=torch.uint8).reshape(
                self.input_size, self.input_size, 3
            )
            x = (x.float() / 255.0).permute(2, 0, 1).unsqueeze(0)
